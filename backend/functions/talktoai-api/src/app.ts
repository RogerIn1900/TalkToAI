...
import { inferMarketQuery } from "./market-query";
import { CloudBaseQuotaStore, CloudBaseSqlQuotaStore, MemoryQuotaStore } from "./quota";
import type { AttachmentRef, Bar, ChatMessage, MarketDataProvider, MarketEnvelope, QuotaStore } from "./types";
...
async function handleChat(req: IncomingMessage, res: ServerResponse, deps: AppDependencies, requestId: string): Promise<void> {
  ...
  const marketRequest = inferMarketQuery(input.messages);
  let market: MarketResult | undefined;
  if (marketRequest && marketRequest.symbols.length === 1) {
    ...
    const additional = await Promise.allSettled(marketRequest.symbols.map((symbol) =>
    ...
    if (!available.length) throw new ApiError(503, "MARKET_UNAVAILABLE", "请求的行情数据暂不可用，请重试；不会生成虚构行情", true);
    market = {
      ...available[0]!,
      overview: available,
