...
import type { Bar, MarketDataProvider, MarketEnvelope, Period } from "./types";
...
export class FixtureMarketDataProvider implements MarketDataProvider {
  ...
  async bars(symbol: string, period: Period, from: string | undefined, to: string | undefined, now: Date): Promise<MarketEnvelope<Bar[]>> {
    ...
    if (from) bars = bars.filter((bar) => bar.time.slice(0, 10) >= from);
    if (to) bars = bars.filter((bar) => bar.time.slice(0, 10) <= to);
    return this.envelope(symbol, period, now, bars);
  }

  private intradayBars(): Bar[] {
    return [
  ...
  private envelope<T>(symbol: string, period: Period, now: Date, data: T): MarketEnvelope<T> {
...
function aggregateBars(bars: Bar[], period: "week" | "month"): Bar {
  const first = bars[0]!;
