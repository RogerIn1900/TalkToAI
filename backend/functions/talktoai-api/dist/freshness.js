...
function evaluateFreshness(input) {
    ...
    if (age < 0)
        return { status: "UNKNOWN", reason: "数据时间晚于设备时间" };
    if (input.period === "intraday") {
        if (input.isTradingSession) {
            if (age <= ONE_MINUTE_MS)
                return { status: "FRESH", reason: "交易时段内数据延迟不超过60秒" };
            if (age <= FIVE_MINUTES_MS)
                return { status: "DELAYED", reason: "交易时段内数据延迟为1至5分钟" };
    ...
    const marketDay = input.marketTime.toISOString().slice(0, 10);
    if (marketDay < input.latestTradingDay)
        return { status: "STALE", reason: "缺少最近交易日数据" };
