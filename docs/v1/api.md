...
模型正文传输协议仍为 Markdown 文本，不是 JSON 或 XML。Android 端不显示 Markdown 标记：标题、列表、段落和代码块转换为原生 Kuikly 样式，满足数值要求的 Markdown 表格转换为结构化图表（默认折线，可切换柱状和饼状），表格源码不进入对话展示。只有用户明确要求原始结构化数据时，模型才可返回 JSON/XML 代码块。
